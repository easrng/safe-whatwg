...
import _mappingTable from "./mappingTable.json" with {
  type: "json",
};
const mappingTable = _mappingTable as [
...
const regex_combiningMarks = new SafeRegExp(
...
const regex_combiningClassVirama = new SafeRegExp(
...
const regex_validZWNJ = new SafeRegExp(
...
const regex_bidiDomain = new SafeRegExp(
...
const regex_bidiS1LTR = new SafeRegExp(
...
const regex_bidiS1RTL = new SafeRegExp(
...
const regex_bidiS2 = new SafeRegExp(
...
const regex_bidiS3 = new SafeRegExp(
...
const regex_bidiS6 = new SafeRegExp(
...
const regex_nonASCII = new SafeRegExp(/[^\x00-\x7F]/u);

function containsNonASCII(str: string) {
  return RegExpPrototypeTest(regex_nonASCII, str);
}
...
function validateLabel(
  ...
  if (RegExpPrototypeTest(regex_combiningMarks, codePoints[0]!)) {
    return false;
  }
  ...
  if (checkBidi && isBidi) {
    ...
    // 1
    if (RegExpPrototypeTest(regex_bidiS1LTR, codePoints[0]!)) {
      rtl = false;
    } else if (RegExpPrototypeTest(regex_bidiS1RTL, codePoints[0]!)) {
      rtl = true;
    } else {
      return false;
    }
    ...
    if (rtl) {
      // 2-4
      if (
        !RegExpPrototypeTest(regex_bidiS2, label) ||
        !RegExpPrototypeTest(regex_bidiS3, label) ||
        (RegExpPrototypeTest(regex_bidiS4EN, label) &&
          RegExpPrototypeTest(regex_bidiS4AN, label))
      ) {
        return false;
      }
    } else if (
      !RegExpPrototypeTest(regex_bidiS5, label) ||
      !RegExpPrototypeTest(regex_bidiS6, label)
    ) {
      // 5-6
      return false;
    }
...
function isBidiDomain(labels: string[]) {
  ...
  return RegExpPrototypeTest(regex_bidiDomain, domain);
}
